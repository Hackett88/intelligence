 ...
 * strategy_pages / strategy_bindings 两张表（迁移 0014）。
 *
 *   · 绑定按「关键词文本 + 市场」自然键存（不引用静态词 id）；读回时用 ALL_KEYWORDS
 *     反查回 kwId 交给前端（前端状态仍以 kwId 为键，UI/reducer 不变）。
 *   · 保存采用「整组覆盖」：先删该 owner 全部行再插当前完整状态 —— 天然无孤儿数据。
 *
...
import type { WbPage, Market, PageRole, PlanStatus, PagePlanningIntent, Territory, PlanPayload } from "./_workbench";
...
const ID_BY_NK = new Map(ALL_KEYWORDS.map((k) => [nk(k.keyword, k.market), k.id]));
...
export async function loadPlan(owner: string): Promise<PlanPayload | null> {
  ...
  for (const b of bindingRows) {
    const kwId = ID_BY_NK.get(nk(b.keyword, b.market === "" ? null : b.market));
