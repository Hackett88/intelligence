/**
 * 建表 gsc_page_optimizations —— 页面「内容优化」追踪(每 URL 一行:版本历史数组)。
 *
 * 用途:曝光高点击低的页做内容优化后,在抽屉「基本信息」卡点「标记优化」记一个版本
 * (版本号 + 洛杉矶日历日 + 备注);流量趋势按各版本起算日展示「优化后累计」与总计对比。
 * events 为 jsonb 数组 [{ v, at, note }],单行存该 URL 全部版本历史。
 *
 ...
 *
 * 运行:  npx tsx scripts/ddl-page-optimizations.ts
...
(async () => {
  ...
  try {
    ...
    // ② 建表(幂等)
    await sql.unsafe(`
      CREATE TABLE IF NOT EXISTS gsc_page_optimizations (
    ...
    `);
    console.log("[2/3] CREATE TABLE IF NOT EXISTS gsc_page_optimizations -> OK");

    // ③ 验证:表存在 + 列数对
    const cols = await sql<{ column_name: string }[]>`
      SELECT column_name FROM information_schema.columns
      WHERE table_name = 'gsc_page_optimizations' ORDER BY ordinal_position
    `;
    console.log(`[3/3] 列: ${cols.map((c) => c.column_name).join(", ")}`);

    if (cols.length === 4) {
      console.log("\nVerdict: PASS — gsc_page_optimizations 已建(4 列)。");
    } else {
